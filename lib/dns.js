    ...
    findDomainBySuffix(req, function(err, domain) {
        if (err) {
            ...
            request.get({
              url: url,
              rejectUnauthorized: false
            }, handleListRecordResult.bind(null, domain, deferred));
        }
    });
    return deferred.promise;
}
...
function setRecord(fqdn, type, value, ttl, isDeleteRecord) {
    debug("setRecord:++");
    var deferred = Q.defer();
    async.waterfall([
            ...
            listRecords(domain.name).then(function(records) {
                ...
                for (i = 0; i < records.length; i++) {
                    if (name === records[i].name.toLowerCase() &&
                        ...
                        value.toLowerCase() === records[i].value.toLowerCase()) {
                        record = records[i];
                        break;
                    }
                }
                ...
                if (record) {
                    if (isDeleteRecord === true) //delete item
                        {
                          records.splice(i, 1);
                        }
                    else {
...
function updateRecordsWithoutRetry(domain, records) {
    ...
    if (oriList.length < records.length) {
        ...
        // Check if an array contains a specific object or not
        function myIndexOf(o) {
          for (var i = 0; i < records.length; i++) {
            if (records[i].name == o.name && records[i].type == o.type && records[i].value == o.value && records[i].ttl == o.ttl) {
                return i;
            }
          }
          return -1;
        ...
        var deleteItem = oriList.filter(function(item) {
          return myIndexOf(item) < 0;
        });

        opt.qs = {
            dnsaction: "delete",
            name: deleteItem[0].name,
            records: deleteItem[0].type,
            ttl: deleteItem[0].ttl,
            value: deleteItem[0].value
        };
    }
...
function clearRecord(fqdn, type, value) {
    return setRecord(fqdn, type, value, null, true);
}
